...
import com.lahsuak.apps.mytask.ui.adapters.CategoryAdapter
...
import com.lahsuak.apps.mytask.util.AppConstants.REM_KEY
import com.lahsuak.apps.mytask.util.Util.setClipboard
import com.lahsuak.apps.mytask.util.Util.showReminder
import com.lahsuak.apps.mytask.util.Util.showSubTaskReminder
...
class AddUpdateTaskFragmentDialog : BottomSheetDialogFragment() {
    ...
    private fun setCategoryMenu() {
        val adapter = CategoryAdapter(requireContext(), MyTaskApp.categoryTypes)
        ...
        if (args.taskId == -1 && !args.navigateFromSubtask) {
            ...
            if (!binding.txtRename.text.isNullOrEmpty()) {
                if (binding.txtReminder.text == getString(R.string.add_date_time)) {
                    task = Task(
                        ...
                        color = MyTaskApp.categoryTypes[selectedCategoryPosition].order
                    )
                }
            ...
            if (binding.txtRename.text.trim().isNotEmpty()) {
                if (!args.navigateFromSubtask) {
                    ...
                    taskViewModel.update(task.copy(color = MyTaskApp.categoryTypes[selectedCategoryPosition].order))
                } else {
                    //update subtask
                    subTask.apply {
                        subTitle = binding.txtRename.toTrimString()
